...
```text
文本版链路：
存储 Flash/SSD --加载--> RAM/Cache --取指/读写--> CPU/Core
CPU/Core --总线 Bus/AXI/AHB/APB--> 外设 GPIO/UART/SPI
外设 --输入/输出事件--> 人 / 云 / 设备
```
...
文本版链路：
PC 指向下一条指令
  -> Fetch 从 I-Cache / RAM 取指
  -> Decode 解析操作码和操作数
  -> Execute 由 ALU 或 Load/Store 执行
  -> Registers 保存中间值、参数、返回值和状态
```
...
| 区域 | 典型内容 | 主要风险 |
|---|---|---|
| Stack 栈 | 局部变量、返回地址、保存寄存器 | 栈溢出、返回地址损坏 |
| Heap 堆 | `malloc/new` 动态内存 | 碎片、泄漏、Use After Free |
| BSS | 未初始化全局/静态变量 | 启动清零失败、越界污染 |
| Data | 已初始化全局/静态变量 | 被误写后长期影响系统状态 |
| Rodata | 字符串常量、只读表 | 错误写入会触发异常或未定义行为 |
| Text | 机器指令 | 跑飞、非法指令、执行区权限错误 |
...
正常布局：a[0] | a[1] | a[2] | a[3] | other_var
错误写入：a[4] 实际落到数组外，可能覆盖 other_var 或其他栈/堆数据。

关键点：变量名不能证明内存安全，必须看真实地址、长度和写入范围。
```
...
| 地址范围示例 | 区域 | 典型用途 |
|---|---|---|
| `0x00000000` | Flash/ROM | 取指令、读常量 |
| `0x20000000` | RAM | 变量、栈、堆 |
| `0x40000000` | 外设寄存器 | UART/GPIO/SPI/I2C 等 MMIO |
| `0xE0000000` | 系统控制寄存器 | SysTick、NVIC、MPU 等 |
...
RAM 字节范围：
- B0  ~ B31 : cache line 0
- B32 ~ B63 : cache line 1

如果 DMA 缓冲区没有按 cache line 对齐，clean/invalidate 可能影响相邻数据。
```
...
1. 保留旧文件 config.json。
2. 写入新文件 config.tmp。
3. fsync(config.tmp)，确认新内容落盘。
4. rename(config.tmp, config.json)，用原子替换切换到新版本。
5. fsync 目录，降低掉电后目录项丢失风险。
```
